...
namespace Enjoying.Logging.Adapters
{
    public class SerilogAdapter : ILoggerAdapter, IDisposable
    {
        ...
        {
            var server = _loggingOptions.LogServer.Servers.FirstOrDefault(item => item.Name.ToLower() == "seq");
            if (server != null)
            {
                try
                {
                ...
                catch (Exception ex) { } //silently continue seq configuration
            }
        }

        private void TryRestConfiguration(LoggerConfiguration loggerConfiguration)
        {
            var server = _loggingOptions.LogServer.Servers.FirstOrDefault(item => item.Name.ToLower() == "rest");
            if (server != null)
            {
                try
                {
                ...
                catch (Exception ex) { } //silently continue REST configuration
            }
        }
